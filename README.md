...
- `src/DemoAddDotNet/ClientApp/` - Web Application folder
...
The service worker initialization code is commented out by default. To enable it, uncomment the following code in `src/DemoAddDotNet/ClientApp/src/index.html`:
...
In ./src/DemoAddDotNet/ClientApp run
...
auto-refreshes when files change on your hard drive.
    ...
    npm --prefix ./src/DemoAddDotNet/ClientApp start

npm is also used to manage CSS and JavaScript dependencies used in this application. You can upgrade dependencies by
specifying a newer version in [package.json](package.json). You can also run `npm update` and `npm install` to manage dependencies.
Add the `help` flag on any command to see how you can use it. For example, `npm help update`.

The `npm --prefix ./src/DemoAddDotNet/ClientApp run` command will list all of the scripts available to run for this project.

## Building for production

### .Net Production builds

To build the artifacts and optimize the DemoAddDotNet application for production, run:

```
cd ./src/DemoAddDotNet
rm -rf ./src/DemoAddDotNet/wwwroot
dotnet publish --verbosity normal -c Release -o ./app/out ./DemoAddDotNet.csproj
```

The `./src/DemoAddDotNet/app/out` directory will contain your application dll and its depedencies.

## Testing

### Client tests

Unit tests are run by [Jest][]. They're located in [src/DemoAddDotNet/ClientApp/test/](src/DemoAddDotNet/ClientApp/test/) and can be run with:
...
3. Run ``dotnet sonarscanner begin /d:sonar.login=admin /d:sonar.password=admin /k:"DemoAddDotNet" /d:sonar.host.url="http://localhost:9001" /s:"`pwd`/SonarQube.Analysis.xml"``
...
```bash
docker build -f ./Dockerfile-Back -t demoadddotnet .
```

Then run:

```bash
docker run -p 8080:8080 demoadddotnet
```
