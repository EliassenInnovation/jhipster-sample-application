...
public class NotificationResource {
        ...
        Optional<Notification> result = notificationRepository
            ...
            .map(existingNotification -> {
                ...
                if (notification.getCreatedOn() != null) {
                    existingNotification.setCreatedOn(notification.getCreatedOn());
                }
     ...
     */
    @GetMapping("/_search")
    public List<Notification> searchNotifications(@RequestParam("query") String query) {
        LOG.debug("REST request to search Notifications for query {}", query);
        try {
            return StreamSupport.stream(notificationSearchRepository.search(query).spliterator(), false).toList();
