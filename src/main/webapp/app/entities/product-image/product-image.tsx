...
import { getEntities, searchEntities } from './product-image.reducer';
  ...
  return (
    <div>
      <h2 id="product-image-heading" data-cy="ProductImageHeading">
        <Translate contentKey="dboApp.productImage.home.title">Product Images</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="dboApp.productImage.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/product-image/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="dboApp.productImage.home.createLabel">Create new Product Image</Translate>
          </Link>
        </div>
      </h2>
      <Row>
            ...
            <FormGroup>
              <InputGroup>
                <Input
                  ...
                  placeholder={translate('dboApp.productImage.home.search')}
                />
        ...
        {productImageList && productImageList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="dboApp.productImage.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
            ...
            <tbody>
              {productImageList.map((productImage, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  <td>
                    <Button tag={Link} to={`/product-image/${productImage.id}`} color="link" size="sm">
                      {productImage.id}
                    </Button>
                  </td>
                  <td>{productImage.createdBy}</td>
                  <td>
                    {productImage.createdOn ? (
                      <TextFormat type="date" value={productImage.createdOn} format={APP_LOCAL_DATE_FORMAT} />
                    ) : null}
                  </td>
                  <td>{productImage.imageURL}</td>
                  <td>{productImage.isActive ? 'true' : 'false'}</td>
                  <td>{productImage.productId}</td>
                  <td>{productImage.productImageId}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`/product-image/${productImage.id}`} color="info" size="sm" data-cy="entityDetailsButton">
                        <FontAwesomeIcon icon="eye" />{' '}
                        <span className="d-none d-md-inline">
                          <Translate contentKey="entity.action.view">View</Translate>
                        </span>
                      </Button>
                      <Button tag={Link} to={`/product-image/${productImage.id}/edit`} color="primary" size="sm" data-cy="entityEditButton">
                      ...
                      <Button
                        onClick={() => (window.location.href = `/product-image/${productImage.id}/delete`)}
                        color="danger"
                        size="sm"
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="dboApp.productImage.home.notFound">No Product Images found</Translate>
            </div>
          )
        )}
      </div>
